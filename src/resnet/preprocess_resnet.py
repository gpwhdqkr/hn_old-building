# ============================================================
# [사용법]
# 이 파일은 ResNet 학습에 쓰는 데이터셋/데이터로더 정의 파일입니다.
...
#
# [기존 코드와의 관계 — 공정 비교를 위해 전처리를 일부러 똑같이 맞춤]
# - transform(증강/정규화)은 팀원의 preprocess_data.py, EfficientNet의
#   preprocess_efficientnet.py와 완전히 동일
# - split도 팀원의 metadata_split.csv를 그대로 사용 (재분리 안 함)
# → 따라서 test 결과 차이는 "모델 차이"로만 해석할 수 있음.
#   여기를 건드리면 기존 실험들과 숫자를 나란히 놓을 수 없게 되므로 수정 금지.
#
# [EfficientNet 버전과 다른 점]
# - batch_size를 BATCH_SIZE 환경변수로 받음 (기본 128).
#   ResNet50은 EfficientNet-B0보다 activation 메모리를 훨씬 많이 써서 VRAM이 작은
#   팟에서는 OOM이 날 수 있음. 그때 코드 수정 없이 BATCH_SIZE=64로 낮추면 됨:
#     BATCH_SIZE=64 python src/resnet/train_resnet.py
# ============================================================
...
# ImageNet 사전학습 모델 표준 평균/표준편차 (팀원/EfficientNet과 동일)
image_mean = [0.485, 0.456, 0.406]
image_std = [0.229, 0.224, 0.225]

# 한 번에 모델에 전달할 이미지 수
# 기본 128은 20GB VRAM 기준 ResNet18 + 224x224에서 여유 있는 값.
# ResNet50에서 OOM이 나면 실행 시 BATCH_SIZE=64 로 낮추면 됨 (코드 수정 불필요)
...
# Train 이미지 전처리 (팀원/EfficientNet과 동일 — 수정 금지)
...
def build_dataloaders():
    ...
    사용 예)
        from preprocess_resnet import build_dataloaders
