# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) MobileNetV2 baseline 학습 스크립트 (1단계).
# ImageNet 사전학습 가중치를 불러와 특징 추출부(features)는 전부 동결하고
# 분류층(classifier)만 우수/불량 2클래스 분류로 학습합니다.
#
...
#   BATCH_SIZE  : 배치 크기 (기본 50 — 기존 MobileNetV2 실험과 동일한 공정 비교 조건)
#   DATA_DIR    : 데이터가 다른 위치에 있을 때만 지정
#
# 결과:
#   model/best_mobilenet_v2_binclf_baseline_<RUN_NAME>.pth
#   (validation macro F1이 가장 높았던 시점의 모델이 저장됨)
#
# 다음 단계 (같은 RUN_NAME을 반드시 그대로 넘겨야 함):
#   RUN_NAME=bin1 python src/binclf/mobilenet/fine_tune_binclf_mobilenet.py
#
# [기존 3클래스 실험과의 비교 조건]
# - 전처리/split/옵티마이저(Adam, lr=0.001)/동결 범위(features 전체 동결)/배치 50은
#   기존 src/train_model.py와 동일. 다른 것은 ① 라벨 정의(보통→불량 병합)
#   ② 클래스 가중치(이원화 train 분포에서 재계산 — 하드코딩 3클래스 값은 사용 불가)
#   ③ 최고모델 선택 기준.
...
def create_mobilenet_model(use_pretrained_weights):
    """MobileNetV2를 만들고 분류층을 2클래스 출력으로 교체해서 반환한다.
...
def main():
    ...
    # ImageNet으로 사전학습된 MobileNetV2 생성 (분류층은 2클래스로 교체된 상태)
    model = create_mobilenet_model(use_pretrained_weights=True)

    # 특징 추출부 전체 동결 (분류층만 학습하는 baseline — 기존 src/train_model.py와 동일)
    ...
    # 분류층만 학습 (기존 src/train_model.py와 동일한 lr)
    optimizer = Adam(
