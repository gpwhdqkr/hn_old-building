# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) MobileNetV2 학습에 쓰는 데이터셋/데이터로더 정의 파일입니다.
# 보통은 직접 실행할 필요 없이 train/fine_tune/evaluate 스크립트가 import해서 씁니다.
#
# 데이터 로딩이 정상 동작하는지 확인하고 싶으면 아래처럼 직접 실행:
#   python src/binclf/mobilenet/preprocess_binclf_mobilenet.py
#
# [전제 조건]
...
#
# [기존 MobileNetV2 코드와의 관계]
# 기존 src/train_model.py와 학습 조건(동결 범위/lr/batch 50)은 동일하나
# 코드 구조는 binclf 표준(resnet 스타일)을 따름.
# (기존 src/preprocess_data.py는 부작용 import·basename 매칭 등 구식 스타일이라
#  구조는 계승하지 않고, batch_size=50 등 학습 조건만 이식함)
#
# [기존 3클래스 코드와의 관계 — 공정 비교를 위해 전처리를 일부러 똑같이 맞춤]
# - transform(증강/정규화)은 기존 3클래스 실험과 완전히 동일
...
#
# [환경변수]
#   BATCH_SIZE      : 배치 크기 (기본 50 — 기존 src/preprocess_data.py의 batch_size=50과
#                     동일하게 맞춘 공정 비교 조건. MobileNetV2는 가벼워서 GPU 여유가
#                     있으면 BATCH_SIZE=128 등으로 상향 가능하나, 기존 실험과 나란히
#                     비교하려면 기본값 50을 유지할 것)
...
# 이 파일이 src/binclf/mobilenet/ 안에 있으므로 네 단계 올라가면 프로젝트 폴더
...
# 한 번에 모델에 전달할 이미지 수
# (기본 50 — 기존 src/preprocess_data.py의 MobileNetV2 실험과 동일한 공정 비교 조건)
...
def build_dataloaders():
    ...
    사용 예)
        from preprocess_binclf_mobilenet import build_dataloaders
