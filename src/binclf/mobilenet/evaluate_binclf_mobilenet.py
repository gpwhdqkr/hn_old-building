# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) MobileNetV2 모델을 Test 데이터로 최종 평가하는
# 스크립트 (3단계). 클래스별 F1/정확도/혼동행렬에 더해 **추론속도**를 함께 측정합니다.
...
#   python src/binclf/mobilenet/evaluate_binclf_mobilenet.py
#
# [환경변수]
#   RUN_NAME    : 실험 이름 (기본 "default") — 학습 때와 같은 값
#   EVAL_TARGET : "finetuned" (기본) 또는 "baseline" — 어느 단계 모델을 평가할지
#   BATCH_SIZE  : 배치 크기 (기본 50) — 처리량 측정에도 이 값이 쓰임
#   DATA_DIR    : 데이터가 다른 위치에 있을 때만 지정
#
# 결과:
#   test_results/binclf/mobilenet_v2/<RUN_NAME>/evaluate_<EVAL_TARGET>_result.txt
#   test_results/binclf/mobilenet_v2/<RUN_NAME>/test_confusion_matrix_<EVAL_TARGET>.csv
#   (화면에 나온 내용이 그대로 txt로도 저장되므로 tee가 필요 없음)
#
# [추론속도를 세 가지로 나눠 재는 이유]
# dataloader 루프 전체를 감싼 수치 하나에는 이미지 파일 읽기/전처리 시간이 섞여 있어
...
#
# [기존 결과와 비교하는 법]
# 이원화(2클래스) 결과이므로 기존 3클래스 결과와 accuracy를 직접 비교하면 안 됩니다.
# (보통이 불량에 병합되어 문제 자체가 다름) 같은 이원화 실험인
# binclf/resnet 결과와 나란히 비교하고, 핵심 지표는 "불량" 행의 f1-score입니다
# (프로젝트 목표 = 불량 탐지, 불량이 positive).
# ============================================================
...
# 결과 저장 위치: test_results/binclf/mobilenet_v2/<실험 이름>/ 아래
# (기존 3클래스 결과 test_results/<arch>/ 와 폴더가 나뉘어 서로 덮어쓰지 않음)
...
def create_mobilenet_model():
    ...
    train_binclf_mobilenet.py의 create_mobilenet_model과 반드시 동일한 구조여야
    로드가 성공한다.
    """
...
def main():
    ...
    record("===== 평가 대상 =====")
    record(f"모델 종류 (MODEL_ARCH) : {MODEL_ARCH}")
    ...
    # 이원화에서는 소수 클래스인 우수를 불량으로 잘못 보는 방향이 병목이라 따로 표시
    ...
    record(f"Test 우수 F1 (참고)   : {test_good_f1:.4f}")
