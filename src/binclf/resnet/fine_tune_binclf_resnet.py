# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) ResNet 파인튜닝 스크립트 (2단계).
# 1단계 baseline 모델을 불러와서, 마지막 특징 블록(layer4)까지 동결을 풀고
# 아주 작은 학습률로 조금 더 학습합니다.
#
# 실행 (반드시 train_binclf_resnet.py를 먼저 실행해야 함. 런팟 = 리눅스):
...
#   python src/binclf/resnet/fine_tune_binclf_resnet.py
#
# [환경변수] — train_binclf_resnet.py 실행 때와 반드시 같은 값을 써야 그 baseline을 찾습니다
...
#
# 결과:
#   model/best_<RESNET_ARCH>_binclf_finetuned_<RUN_NAME>.pth
...
#
# [기존 3클래스 실험과의 차이]
# 동결 해제 범위(layer4+fc)/차등 학습률(1e-5/1e-4)/patience(3)는 기존과 동일.
# 다른 것은 라벨 정의(이원화), 클래스 가중치(분포 재계산), 선택 기준(macro F1)뿐.
# ============================================================
...
# 학습할 ResNet 종류: train_binclf_resnet.py 실행 때와 반드시 같은 값
RESNET_ARCH = os.environ.get("RESNET_ARCH", "resnet18")

# 실험 이름: train_binclf_resnet.py 실행 때와 반드시 같은 값을 써야 baseline을 찾음
RUN_NAME = os.environ.get("RUN_NAME", "default")

# 1단계에서 저장한 baseline 모델
baseline_model_path = model_dir / f"best_{RESNET_ARCH}_binclf_baseline_{RUN_NAME}.pth"

# 파인튜닝 결과를 저장할 경로
fine_tuned_model_path = model_dir / f"best_{RESNET_ARCH}_binclf_finetuned_{RUN_NAME}.pth"
...
def create_resnet_model():
    ...
    (weights=None). train_binclf_resnet.py의 같은 이름 함수와 반드시 동일한 구조를
...
def main():
    ...
    if not baseline_model_path.exists():
        raise FileNotFoundError(
            ...
            "먼저 같은 RESNET_ARCH와 RUN_NAME으로 train_binclf_resnet.py를 실행하세요.\n"
    ...
    # (기존 fine_tune_resnet.py와 동일한 차등 학습률)
    optimizer = Adam(
        [
            {
