...
# — 재학습 없이 기존 체크포인트만 사용.
#
# 학습된 이원화 ResNet 체크포인트를 읽어 validation 셋에서 "불량 확률이
...
# 결과는 test_results/binclf/<RESNET_ARCH>/<RUN_NAME>_threshold/ 아래에만 새로 저장됩니다.
#
# 실행 (런팟 = 리눅스):
#   RESNET_ARCH=resnet18 RUN_NAME=bin1 python src/binclf/resnet/tune_threshold_binclf_resnet.py
#
# 실행 (윈도우 PowerShell):
#   $env:RESNET_ARCH="resnet18"; $env:RUN_NAME="bin1"
#   python src/binclf/resnet/tune_threshold_binclf_resnet.py
#
# [환경변수]
...
#                  (preprocess_binclf_resnet가 읽어서 경로에 반영함)
#
...
results_dir = test_results_root / RESNET_ARCH / f"{RUN_NAME}_threshold"
...
class ThresholdDataset(Dataset):
    ...
    preprocess_binclf_resnet.BuildingDataset과 동일한 동작이지만, 이미지 폴더
...
def create_resnet_model():
    """저장된 체크포인트를 덮어씌울 빈 ResNet 구조를 만든다 (evaluate_binclf_resnet.py와 동일)."""
...
def main():
    ...
    # 동일한 결정 (= 기존 evaluate_binclf_resnet.py 결과와 같아야 정상)
