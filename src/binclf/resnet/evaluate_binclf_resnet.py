# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) ResNet 모델을 Test 데이터로 최종 평가하는 스크립트 (3단계).
# 클래스별 F1/정확도/혼동행렬에 더해 **추론속도**를 함께 측정합니다.
...
#   $env:RESNET_ARCH="resnet18"; $env:RUN_NAME="bin1"; $env:EVAL_TARGET="finetuned"
#   python src/binclf/resnet/evaluate_binclf_resnet.py
#
# [환경변수]
...
#
# 결과:
#   test_results/binclf/<RESNET_ARCH>/<RUN_NAME>/evaluate_<EVAL_TARGET>_result.txt
#   test_results/binclf/<RESNET_ARCH>/<RUN_NAME>/test_confusion_matrix_<EVAL_TARGET>.csv
...
#
# [이원화 결과를 읽을 때 주의 — accuracy에 속지 말 것]
# 보통을 불량에 병합했기 때문에 test는 불량이 약 91%입니다. 전부 불량으로만
# 찍는 무의미한 모델도 accuracy가 약 91%로 나오므로, accuracy는 참고용일
# 뿐입니다. 모델이 우수를 실제로 구분해내는지를 보여주는 우수 F1과
# macro F1을 기준으로 판단하세요. (불량 F1은 분포 특성상 높게 나오기 쉬움)
# ============================================================
...
# 경로/데이터로더는 이원화 공용 정의를 그대로 사용 (기존 3클래스와 폴더가 분리됨)
...
# 평가할 모델 경로 ("binclf" 토큰이 있어 기존 3클래스 모델과 절대 겹치지 않음)
model_path = (
    model_dir
    / f"best_{RESNET_ARCH}_binclf_{EVALUATION_TARGET}_{RUN_NAME}.pth"
)

# 결과 저장 위치: test_results/binclf/<모델명>/<실험 이름>/ 아래
# (기존 test_results/<arch>/ 3클래스 결과와 폴더가 나뉘어 서로 덮어쓰지 않음)
test_results_dir = test_results_root / RESNET_ARCH / RUN_NAME
...
# 모델 출력 순서와 클래스 이름 (이원화: 0=우수, 1=불량)
class_labels = [0, 1]
class_names = ["우수", "불량"]
...
def create_resnet_model():
    """RESNET_ARCH에 맞는 ResNet 구조를 만들고 분류층을 2클래스 출력으로 교체해서 반환한다.

    저장된 가중치를 덮어씌울 것이므로 ImageNet 가중치는 받지 않는다 (weights=None).
    train_binclf_resnet.py의 같은 이름 함수와 반드시 동일한 구조여야 로드가 성공한다.
    """
...
def main():
    ...
    # 이원화 학습은 최고모델 선택 기준이 validation macro F1이므로 함께 표시
    saved_validation_macro_f1 = checkpoint.get("validation_macro_f1")
    if isinstance(saved_validation_macro_f1, float):
        record(f"저장 당시 Validation Macro F1 : {saved_validation_macro_f1:.4f}")
    ...
    # 단, 헤더에 적었듯 이원화에서는 불량 F1/accuracy가 높게 나오기 쉬우므로
    # 우수 F1과 macro F1을 함께 봐야 한다.
    test_defect_f1 = per_class_f1[1]
    test_good_f1 = per_class_f1[0]
    ...
    record(f"우수 F1 : {test_good_f1:.4f}")
    ...
    # 우수를 불량으로 잘못 본 장수 — 이원화에서 accuracy로는 보이지 않는 병목이라 따로 표시
    # (우수 recall과 표리 관계: 이 비율이 낮을수록 우수 recall이 높다)
    good_as_defect = int(matrix[0][1])
    good_total = int(matrix[0].sum())

    record("")
    record(
        f"우수 {good_total}장 중 {good_as_defect}장을 불량으로 오분류 "
    ...
    record("")
    record("==========================================")
    record(f"[비교용 최종 지표] {RESNET_ARCH} binclf ({EVALUATION_TARGET})")
    record(f"Test 불량 F1 (핵심)   : {test_defect_f1:.4f}")
    record(f"Test 우수 F1          : {test_good_f1:.4f}")
