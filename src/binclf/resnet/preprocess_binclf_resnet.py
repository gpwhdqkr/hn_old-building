# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) ResNet 학습에 쓰는 데이터셋/데이터로더 정의 파일입니다.
...
# - transform(증강/정규화)은 src/resnet/preprocess_resnet.py와 완전히 동일
...
#
# [환경변수]
#   BATCH_SIZE      : 배치 크기 (기본 128). resnet50 OOM 시 64로 낮출 것
...
# 이 파일이 src/binclf/resnet/ 안에 있으므로 네 단계 올라가면 프로젝트 폴더
...
# 한 번에 모델에 전달할 이미지 수 (기존 ResNet 실험과 동일한 기본값)
...
def build_dataloaders():
    ...
    사용 예)
        from preprocess_binclf_resnet import build_dataloaders
