# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) EfficientNet-B0 모델을 Test 데이터로 최종 평가하는 스크립트 (3단계).
...
#   python src/binclf/efficientnet/evaluate_binclf_efficientnet.py
#
# [환경변수]
...
#
# 결과:
#   test_results/binclf/efficientnet_b0/<RUN_NAME>/evaluate_<EVAL_TARGET>_result.txt
#   test_results/binclf/efficientnet_b0/<RUN_NAME>/test_confusion_matrix_<EVAL_TARGET>.csv
...
# 결과 저장 위치: test_results/binclf/efficientnet_b0/<실험 이름>/ 아래
# (기존 test_results/<arch>/ 3클래스 결과와 폴더가 나뉘어 서로 덮어쓰지 않음)
test_results_dir = test_results_root / MODEL_ARCH / RUN_NAME
...
def create_efficientnet_model():
    """EfficientNet-B0 구조를 만들고 분류층을 2클래스 출력으로 교체해서 반환한다.

    저장된 가중치를 덮어씌울 것이므로 ImageNet 가중치는 받지 않는다 (weights=None).
    train_binclf_efficientnet.py의 같은 이름 함수와 반드시 동일한 구조여야 로드가 성공한다.
    """
...
def main():
    ...
    record("")
    record("==========================================")
    record(f"[비교용 최종 지표] {MODEL_ARCH} binclf ({EVALUATION_TARGET})")
