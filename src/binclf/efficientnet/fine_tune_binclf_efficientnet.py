# ============================================================
# [사용법]
# 이원화(2클래스: 우수 vs 불량) EfficientNet-B0 파인튜닝 스크립트 (2단계).
# 1단계 baseline 모델을 불러와서, 마지막 특징 블록(features[-1])까지 동결을 풀고
# 아주 작은 학습률로 조금 더 학습합니다.
#
# 실행 (반드시 train_binclf_efficientnet.py를 먼저 실행해야 함. 런팟 = 리눅스):
...
#   python src/binclf/efficientnet/fine_tune_binclf_efficientnet.py
#
# [환경변수] — train_binclf_efficientnet.py 실행 때와 반드시 같은 값을 써야 그 baseline을 찾습니다
...
#
# 결과:
#   model/best_efficientnet_b0_binclf_finetuned_<RUN_NAME>.pth
...
#
# [기존 3클래스 실험과의 차이]
# 동결 해제 범위(features[-1]+classifier)/차등 학습률(1e-5/1e-4)/patience(3)는 기존과 동일.
...
# 실험 이름: train_binclf_efficientnet.py 실행 때와 반드시 같은 값을 써야 baseline을 찾음
...
def create_efficientnet_model():
    ...
    (weights=None). train_binclf_efficientnet.py의 같은 이름 함수와 반드시 동일한
    구조를 만들어야 한다. 한쪽만 바꾸면 baseline 가중치 로드가 실패한다.
    """
...
def main():
    ...
    if not baseline_model_path.exists():
        raise FileNotFoundError(
            ...
            "먼저 같은 RUN_NAME으로 train_binclf_efficientnet.py를 실행하세요.\n"
    ...
    # (기존 fine_tune_efficientnet.py와 동일한 차등 학습률)
    optimizer = Adam(
        [
            {
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # EfficientNet은 특징 블록이 features 컨테이너에 순서대로 있어 마지막만 빼고 순회
