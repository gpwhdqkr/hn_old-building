...
# ============================================================
# v2 공용 학습/평가 엔진.
# 이원화(binclf)와 결함종류 7클래스(midclf)의 train/fine_tune/evaluate 로직이
# 라벨 정의만 다르고 완전히 같기 때문에, 여기에 한 번만 구현하고
# 과제별 스크립트(train_binclf_v2.py 등)는 이 엔진을 task 인자로 호출합니다.
# (같은 로직이 7개 파일에 복제되면 버그 수정이 7곳으로 번지는 것을 방지)
#
# 기존 binclf 대비 v2 신규:
#   - 시드 고정 (preprocess_v2.set_seed)
...
TASK_LABEL_DEFINITIONS = {
    ...
    "midclf": "middle_id 7클래스 (C/P/X/F/T/L/W 순서 고정)",
}
...
class HistoryWriter:
    ...
    focus F1: binclf는 우수(소수 클래스) F1, midclf는 최저 클래스 F1 —
...
def run_fine_tune(task):
    ...
    if not baseline_path.exists():
        raise FileNotFoundError(
            ...
            f"먼저 같은 ARCH/RUN_NAME으로 train_{task}_v2.py를 실행하세요."
        )
...
def run_evaluate(task):
    ...
    if task == "binclf":
        ...
        from preprocess_v2 import MIDDLE_ORDER
        predictions_frame["true_middle"] = [
            MIDDLE_ORDER[label] for label in all_labels
        ]
        predictions_frame["pred_middle"] = [
            MIDDLE_ORDER[prediction] for prediction in all_predictions
        ]
        for class_index, middle in enumerate(MIDDLE_ORDER):
            predictions_frame[f"prob_{middle}"] = [
                round(probability[class_index], 6)
