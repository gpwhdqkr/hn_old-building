...
# ============================================================
# [사용법]
# v2 학습 파이프라인(이원화 binclf_v2 / 결함종류 7클래스 midclf_v2)의
# 공용 데이터 정의 파일입니다. 보통은 train/fine_tune/evaluate 스크립트가
# import해서 쓰고, 데이터 로딩 점검용으로 직접 실행할 수 있습니다:
#
#   (리눅스)     DATA_DIR=/workspace/hn_old-building python src/v2/preprocess_v2.py
#   (PowerShell) $env:DATA_DIR="D:/hn_old-building"; python src/v2/preprocess_v2.py
#   TASK=midclf 로 실행하면 7클래스 기준으로 점검
#
# [전제 조건 — 전처리 v2 산출물]
# - data/processed/metadata_split_v2.csv   (라벨+split 정본, 깃 포함)
# - data/processed/annotations_v2.jsonl    (폴리곤, 깃 포함 여부는 README 참고)
...
#
# [주의]
# - 이 파일은 src/v2/ 안에 있으므로 프로젝트 루트는 두 단계 위입니다
...
# src/v2/xxx.py → 두 단계 위가 프로젝트 루트
project_dir = Path(__file__).resolve().parent.parent.parent

data_dir = Path(os.environ.get("DATA_DIR", project_dir))

metadata_path = data_dir / "data" / "processed" / "metadata_split_v2.csv"
...
# ---- 과제 정의 ----
# binclf: 이원화 (0=우수, 1=불량(보통 병합)) — 극불균형 8.8%/91.2%
# midclf: 결함 종류 7클래스 — 추론은 softmax 최고 확률
BIN_CLASS_NAMES = ["우수", "불량"]

MIDDLE_ORDER = ["C", "P", "X", "F", "T", "L", "W"]
MIDDLE_NAMES = {
    "C": "구조물(균열)",
    "P": "구조물(박리, 박락)",
    "X": "구조물(철근 노출)",
...
MIDDLE_TO_INDEX = {middle: index for index, middle in enumerate(MIDDLE_ORDER)}

TASK_TOKENS = {"binclf": "binclf_v2", "midclf": "midclf_v2"}
TASK_NUM_CLASSES = {"binclf": 2, "midclf": 7}
TASK_CLASS_NAMES = {
    "binclf": BIN_CLASS_NAMES,
    "midclf": [MIDDLE_NAMES[middle] for middle in MIDDLE_ORDER],
}
...
def results_dir(task, arch, run_name):
    """결과 저장 폴더: test_results/<binclf_v2|midclf_v2>/<ARCH>/<RUN_NAME>/"""
...
class BuildingDatasetV2(Dataset):
    ...
    def __init__(self, dataframe, task, split_kind, defect_bboxes=None):
        if task not in TASK_TOKENS:
            raise ValueError(f'task는 "binclf" 또는 "midclf"여야 합니다: {task}')
    ...
    def __getitem__(self, index):
        ...
        if self.task == "binclf":
            ...
            label = MIDDLE_TO_INDEX[row["middle_id"]]
...
def load_split_dataframes(task):
    ...
    found_middles = set(metadata["middle_id"].unique().tolist())
    if not found_middles <= set(MIDDLE_ORDER):
        raise ValueError(f"middle_id에 정의 외 값이 있습니다: {sorted(found_middles)}")
    ...
    # 스모크 절단 기준 컬럼: 과제별 라벨 분포를 보존해야 함
    stratify_column = "model_label" if task == "binclf" else "middle_id"
...
def compute_class_weights(train_dataframe, task):
    ...
    if task == "binclf":
        ...
        counts = train_dataframe["middle_id"].value_counts()
        keys = MIDDLE_ORDER
...
if __name__ == "__main__":
    ...
    label_column = "model_label" if task == "binclf" else "middle_id"
