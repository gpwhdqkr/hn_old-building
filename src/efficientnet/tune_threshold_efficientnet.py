# ============================================================
# [사용법]
# EfficientNet-B0 결정 임계값(threshold) 보정 스크립트 — 재학습 없이 기존 체크포인트만 사용.
# (src/resnet/tune_threshold_resnet.py와 같은 절차의 EfficientNet 버전)
#
# 이미 학습된 체크포인트를 읽어 validation 셋에서 "보통(1)" logit에 더할
# 보정치(bias)를 탐색하고, 확정된 bias 하나로 test를 1회 평가합니다.
# bias 탐색은 validation에서만 하므로 test 정보 누출(leakage)이 없습니다.
#
# 기존 코드/체크포인트/평가 결과는 일절 수정하지 않습니다.
# 결과는 test_results/efficientnet_b0/<RUN_NAME>_threshold/ 아래에만 새로 저장됩니다.
#
# 실행 (런팟 = 리눅스):
#   RUN_NAME=epoch20 python src/efficientnet/tune_threshold_efficientnet.py
#
# 실행 (윈도우 로컬, 데이터가 원본 저장소에 있을 때):
#   $env:RUN_NAME="epoch20"; $env:DATA_DIR="D:/hn_old-building"
#   python src/efficientnet/tune_threshold_efficientnet.py
#
# [환경변수]
...
#
# GPU가 없어도 됩니다. 추론 2회(validation 1회 + test 1회)뿐이라 CPU로도 동작합니다.
# ============================================================
...
from preprocess_efficientnet import RAW_PATH_MARKER, evaluation_transform

project_dir = Path(__file__).resolve().parent.parent.parent

RUN_NAME = os.environ.get("RUN_NAME", "epoch20")
...
def create_efficientnet_model():
    ...
    (evaluate_efficientnet.py와 동일한 구조)"""
...
def main():
    ...
    for bias in bias_candidates:
        ...
        sweep_rows.append({
            "bias": result["bias"],
            "보통_recall": round(result["average_recall"], 4),
    ...
    record("==========================================")
    record("결정 임계값(threshold) 보정 실험 결과 — EfficientNet-B0")
    record("==========================================")
    record(f"모델 파일 : {best_model_path.name}")
    record(f"모델 종류 : efficientnet_b0 ({EVALUATION_TARGET})")
